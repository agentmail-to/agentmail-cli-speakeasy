...
import { AgentmailCliDefaultError } from "../models/errors/agentmail-cli-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | AgentmailCliDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | AgentmailCliDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new AgentmailCliDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new AgentmailCliDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
