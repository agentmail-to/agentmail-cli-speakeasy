...
async function run() {
  const result = await agentmailCli.inboxes.events.list("<id>");
...
import { inboxesEventsList } from "agentmail/funcs/inboxes-events-list.js";
...
async function run() {
  const res = await inboxesEventsList(agentmailCli, "<id>");
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("inboxesEventsList failed:", res.error);
  }
}
