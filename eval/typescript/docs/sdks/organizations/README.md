...
* [organizationsGet](#organizationsget) - Get Organization
...
<!-- UsageSnippet language="typescript" operationID="organizations_get" method="get" path="/v0/organizations" -->
```typescript
...
async function run() {
  const result = await agentmailCli.organizations.organizationsGet();
...
import { organizationsOrganizationsGet } from "agentmail/funcs/organizations-organizations-get.js";
...
async function run() {
  const res = await organizationsOrganizationsGet(agentmailCli);
  if (res.ok) {
